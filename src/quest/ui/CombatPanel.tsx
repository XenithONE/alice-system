...
export function CombatPanel({ combat, monster, player }: { combat: CombatState; monster: MonsterDef; player: RivalView }) {
  ...
  return <aside className="rr-combat rr-panel" aria-label="戦闘">
    ...
    <div className="rr-combat__title"><div><span className="rr-kicker">ROUND {combat.round}</span><h3>{monster.nameJa}</h3><small>{monster.name}</small></div><strong>{combat.monsterHp}/{monster.hp} HP</strong></div>
    <div className="rr-hp-segments" aria-label={`モンスターHP ${combat.monsterHp}/${monster.hp}`}>{Array.from({ length: hpSegments }, (_, i) => <i className={i < hpActive ? "is-active" : ""} key={i} />)}</div>
